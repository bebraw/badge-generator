...
- Rebuild generated CSS and browser JavaScript manually with `npm run build:assets`.
...
The badge UI follows the Tailwind v4 baseline shape: Tailwind input lives in `src/tailwind-input.css`, generated CSS is written to `.generated/styles.css`, the generated browser module is written to `.generated/badge-app.client.js`, and Wrangler runs `npm run build:assets` automatically before local development.

The Lighthouse setup is also generic, but the Worker application gives it a concrete local target. Use `LIGHTHOUSE_URL=http://127.0.0.1:8787 LIGHTHOUSE_SERVER_COMMAND="npm run dev" npm run lighthouse`. Reports are written to `reports/lighthouse/`.
...
The quality gate currently runs the fast gate first, then the Playwright browser tests, then mutation tests. The local and remote CI workflow runs separate fast, browser, and mutation jobs, with repository-shape validation included in the fast job. Local Agent CI runs should go through `npm run ci:local`, which lets Agent CI run independent jobs concurrently while `scripts/ci-install-dependencies.sh` serializes the shared warm `node_modules` install and lets later jobs reuse it. The command also pauses a failed runner for agent retry. Local browser installation should also go through the pinned `npm run playwright:install` script.
